...
$j(document).ready(function(){
    function resizeCenterFrame( ){
        var windowH          = $j(window).height(),
            ...
            bottomContainerH = $j('#bottomContainer').height(),
            $middleContainer  = $j('#middleContainer iframe'),
            middleContainerH = $middleContainer.height();
        middleContainerH =  windowH - (topContainerH + bottomContainerH); 
        middleContainerH -= 12;
        console.log('Middle resized to: ' + middleContainerH + 'px.');
        $middleContainer.height(middleContainerH);
    };
    $j('#topContainer').click(function(e){
        window.location.href = '/VirtualStalker';
    });
    function justifyButtons(){
        var $cont = $j('#modulBtnContainer'),
            $ul   = $j('ul.menu'),
            calc = ($cont.width()-$ul.width())/2;
        //$cont.css('left',calc);
        $cont.width( $ul.width() );
    };
...
;(function(oIndex, $){
    ...
    $(document).ready(function(){
        $('#modulBtnContainer button').click(function(e){
            ...
            queryParams += '_rnd=' + Math.random();
            $('#middleContainer iframe').attr('src',$('a',$itm).attr('href') + queryParams );
